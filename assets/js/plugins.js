...
/*-----------------------------------------
 * jQuery One Page Nav Plugin - @version 3.0.0
...
window.onload = function() {
    var c = document.getElementsByClassName("typewrite");
    for (var b = 0; b < c.length; b++) {
        var d = c[b].getAttribute("data-type");
        var e = c[b].getAttribute("data-period");
        if (d) {
            new TxtType(c[b], JSON.parse(d), e)
        }
    }
...
! function(o) {
    o.fn.loading = function() {
        ...
        o(this).each(function() {
            var c = o(this),
                d = {
                    backgroundColor: c.data("color") ? c.data("color").split(",")[0] : r,
                    progressColor: c.data("color") ? c.data("color").split(",")[1] : a,
                    percent: c.data("percent") ? c.data("percent") : t,
                    duration: c.data("duration") ? c.data("duration") : n
                };
            c.append('<div class="background"></div><div class="rotate"></div><div class="left"></div><div class="right"></div><div class=""><span>' + d.percent + "%</span></div>"), c.find(".background").css("background-color", d.backgroundColor), c.find(".left").css("background-color", d.backgroundColor), c.find(".rotate").css("background-color", d.progressColor), c.find(".right").css("background-color", d.progressColor);
            var s = c.find(".rotate");
            if (setTimeout(function() {
                    s.css({
                        transform: "rotate(" + 3.6 * d.percent + "deg)"
                    })
                }, 1), d.percent > 50) {
                var i = "toggle " + d.duration / d.percent * 50 + "ms step-end",
                    e = "toggle " + d.duration / d.percent * 50 + "ms step-start";
                c.find(".right").css({
                    animation: i,
                    opacity: 1
                }), c.find(".left").css({
 ...
 * https://wowjs.uk - Copyright (c) 2016 Thomas Grainger; Licensed MIT
 * -----------------------------------------*/
! function(a, b) {
